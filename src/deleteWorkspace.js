...
async function deleteEmptyWorkspaces(workspaceToMigrateInto) {
    ...
    for (const project of projects) {
        ...
        if (songs.length > 0) {
            console.log(`  Skipping non-empty project ${project.id}.`);
            continue;
        }

        console.log(`  Deleting empty project ${project.id}...`);


        const res = await fetchWithRetry(`${SUNO_API}/project/trash`, {
            method: 'POST',
            body: JSON.stringify({
        ...
        if (res.ok) {
            console.log(`  Deleted empty project ${project.id}.`);
        }
    }

}

await deleteEmptyWorkspaces('default');
